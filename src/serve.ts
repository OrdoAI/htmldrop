...
function trimSpan(m,s,e){while(s<e&&m.full[s].trim()==="")s++;while(e>s&&m.full[e-1].trim()==="")e--;return {start:s,end:e};}
function anchorFromSelection(m,range){var s=offsetOf(m,range.startContainer,range.startOffset),e=offsetOf(m,range.endContainer,range.endOffset);if(s==null||e==null)return null;var b=trimSpan(m,s,e);s=b.start;e=b.end;if(e<=s)return null;return {exact:m.full.substring(s,e),prefix:m.full.substring(Math.max(0,s-32),s),suffix:m.full.substring(e,e+32)};}

function relocate(){ranges={};if(hlAll)hlAll.clear();if(hlActive)hlActive.clear();model=buildModel();roots().forEach(function(c){if(!c.anchor)return;var loc=locateQuote(model.full,c.anchor);if(!loc)return;var b=trimSpan(model,loc.start,loc.end);if(b.end<=b.start)return;var r=rangeFor(model,b.start,b.end);if(r){ranges[c.cid]=r;if(hlAll&&!c.resolved)hlAll.add(r);}});applyHL();}
...
function rangeRects(r){var raw=[].slice.call(r.getClientRects());return raw.filter(function(a,i){for(var j=0;j<raw.length;j++){if(i===j)continue;var b=raw[j];if(a.left<=b.left&&a.top<=b.top&&a.right>=b.right&&a.bottom>=b.bottom&&(a.left<b.left||a.top<b.top||a.right>b.right||a.bottom>b.bottom))return false;}return true;});}
function cidsAtPoint(x,y){var out=[];for(var i=0;i<comments.length;i++){var c=comments[i];if(c.parentId||c.resolved)continue;var r=ranges[c.cid];if(!r)continue;var rects=rangeRects(r);for(var k=0;k<rects.length;k++){var rc=rects[k];if(x>=rc.left&&x<=rc.right&&y>=rc.top&&y<=rc.bottom){out.push(c);break;}}}return out;}
...
function showTip(list,cid){buildTip(list);tipRef.hidden=false;var r=ranges[cid];if(!r){hideTip();return;}var rc=rangeRects(r)[0]||r.getBoundingClientRect();var tw=tipRef.offsetWidth,th=tipRef.offsetHeight,pad=10,vw=window.innerWidth,vh=window.innerHeight;var x=Math.max(pad,Math.min(rc.left-6,vw-tw-pad));var y=rc.top-th-10;if(y<pad)y=rc.bottom+10;y=Math.max(pad,Math.min(y,vh-th-pad));tipRef.style.left=x+"px";tipRef.style.top=y+"px";}
